...
func (e eqCreateUserParamsMatcher) String() string {
	return fmt.Sprintf("matches arg %v and password %v", reflect.TypeOf(e.arg), e.password)
}
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					...
					Times(1).
					Return(db.User{}, errors.New("InternalError"))
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
...
func TestLoginUserAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					GetUser(gomock.Any(), user.Username).
					Times(1).
					Return(db.User{}, sql.ErrConnDone) // 模拟查询出错
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
		},
		{
			...
			body: gin.H{
				"username": user.Username,
				"password": 123456, // password 应该是 string，这里故意传 int
			},
			buildStubs: func(store *mockdb.MockStore) {
				// 不调用任何mock
			},
